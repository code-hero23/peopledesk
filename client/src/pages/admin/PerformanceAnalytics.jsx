...
const PerformanceAnalytics = () => {
    ...
    return (
        <div className="space-y-8 animate-fade-in pb-12 h-screen flex flex-col overflow-hidden">
            {/* Header - Always Fixed */}
            <div className="flex-shrink-0 flex flex-col xl:flex-row justify-between xl:items-center gap-6 bg-white p-6 rounded-[32px] shadow-xl shadow-slate-200/50 border border-slate-100 relative overflow-hidden group mb-2">
                {/* Decorative Pattern */}
                <div className="absolute top-0 right-0 w-64 h-64 bg-blue-50/30 rounded-full blur-[100px] -mr-32 -mt-32"></div>

                <div className="relative z-10">
                    <div className="flex items-center gap-4 mb-2">
                        <div className="p-3 bg-blue-600 text-white rounded-2xl shadow-lg shadow-blue-200 scale-100">
                            <TrendingUp size={24} strokeWidth={3} />
                        </div>
                        <h2 className="text-3xl xl:text-4xl font-black text-slate-800 tracking-tighter">Performance <span className="text-blue-600">Analytics</span></h2>
                    </div>
                    <p className="text-slate-500 font-bold text-xs max-w-xl leading-relaxed ml-1">
                ...
                <div className="relative z-10 flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                    <div className="flex items-center gap-4 bg-slate-50 p-2 rounded-[20px] border border-slate-200/60 shadow-inner">
                        <div className="flex items-center gap-3 pl-3">
                            <Calendar size={16} className="text-blue-600" />
                            <div className="flex flex-col">
                                <span className="text-[8px] font-black text-slate-400 uppercase tracking-tighter -mb-0.5">Start Date</span>
                                <input
                                    type="date"
                        ...
                        <div className="flex items-center gap-3 pr-3">
                            <div className="flex flex-col text-right">
                                <span className="text-[8px] font-black text-slate-400 uppercase tracking-tighter -mb-0.5 text-right">End Date</span>
                                <input
                                    type="date"
                    ...
                    <button
                        ...
                        className="group flex items-center justify-center gap-2 bg-slate-900 hover:bg-blue-600 text-white px-6 py-3 rounded-[20px] shadow-lg hover:shadow-blue-200 transition-all duration-300 font-black text-xs active:scale-95"
                    >
            ...
            {/* Tab Navigation */}
            <div className="flex-shrink-0 flex gap-4 px-2">
                <button
                    onClick={() => setActiveTab('leaderboard')}
                    className={`px-6 py-3 rounded-2xl font-black text-sm tracking-wide transition-all duration-300 shadow-sm flex items-center gap-2
            ...
            <div className="flex-1 overflow-hidden relative">
                ...
                {activeTab === 'leaderboard' && (
                    <div className="h-full bg-white rounded-[32px] shadow-lg border border-slate-100 flex flex-col overflow-hidden animate-slide-up">
                        <div className="flex-shrink-0 px-8 py-6 bg-white border-b border-slate-50 flex justify-between items-center z-20 relative">
                            <div className="flex items-center gap-4">
                                <div className="p-3 bg-blue-50 text-blue-600 rounded-[18px]">
                                    <Users size={24} strokeWidth={3} />
                                </div>
                                <h3 className="text-xl font-black text-slate-800 tracking-tighter">Ranking Matrix</h3>
                            </div>
                            ...
                            <div className="flex items-center gap-3 bg-slate-50 px-4 py-2.5 rounded-2xl border border-slate-200/60 focus-within:border-blue-500 focus-within:ring-4 focus-within:ring-blue-100 transition-all duration-300">
                                <Search size={16} className="text-slate-400" />
                                <input
                                    type="text"
                                    placeholder="Search team..."
                                    className="bg-transparent border-none outline-none text-xs font-black text-slate-700 placeholder:text-slate-400 w-32 lg:w-48 uppercase tracking-wide"
                                    value={searchTerm}
                            ...
                            <table className="w-full text-left border-separate border-spacing-0">
                                <thead className="sticky top-0 z-30">
                                    <tr className="bg-slate-900 shadow-2xl relative z-10">
                                        <th className="pl-12 pr-6 py-8 text-sm font-black text-slate-400 uppercase tracking-[0.1em] first:rounded-tl-none">
                                            <div className="flex flex-col">
                                        ...
                                        <th className="px-6 py-9 text-right pr-12 last:rounded-tr-none">
                                                    ...
                                                    <div className="flex items-center gap-6">
                                                        ...
                                                        <div>
                                                            <div className="flex items-center gap-3">
                                                                <p className={`font-black text-lg leading-tight transition-colors tracking-tight
                    ...
                    <div className="h-full overflow-y-auto custom-scrollbar pr-2 space-y-8 animate-slide-up pb-10">
                        ...
                        {employeeStats ? (
                            <div ref={statsRef} className="space-y-6">
                                <div className="flex flex-col md:flex-row items-center gap-6 bg-white p-8 rounded-[32px] border border-slate-100 shadow-xl relative overflow-hidden">
                                    <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-blue-600 via-indigo-600 to-violet-600"></div>
                                    <div className="flex items-center gap-6 w-full md:w-auto">
                                        <div className="w-20 h-20 rounded-[28px] bg-gradient-to-br from-blue-600 to-indigo-700 flex items-center justify-center text-white font-black text-3xl shadow-lg ring-8 ring-blue-50">
                                            {teamOverview.find(e => e.id === selectedEmployee)?.name.charAt(0)}
                                        </div>
                                        <div>
                                            <h3 className="text-2xl font-black text-slate-800 tracking-tight">{teamOverview.find(e => e.id === selectedEmployee)?.name}</h3>
                                            <div className="flex items-center gap-2 mt-1">
                                                <span className="px-3 py-1 bg-slate-100 text-slate-500 text-[10px] font-black uppercase tracking-wider rounded-lg">{teamOverview.find(e => e.id === selectedEmployee)?.designation}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex-1 w-full flex justify-end">
                                        <button
                                            ...
                                            className="group flex items-center gap-2 text-slate-400 hover:text-red-500 text-xs font-black uppercase tracking-widest transition-all bg-white hover:bg-red-50 px-5 py-2.5 rounded-xl border border-slate-100 hover:border-red-100 ml-auto md:ml-0"
                                        >
                                ...
                                <div className="bg-white p-8 rounded-[32px] shadow-xl border border-slate-100 relative overflow-hidden">
                                    <div className="flex justify-between items-center mb-6">
                                        <h4 className="font-black text-slate-800 text-xl tracking-tight">Efficiency Trend</h4>
                                    </div>
                        ...
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                            <div className="bg-white p-8 rounded-[32px] shadow-xl border border-slate-100">
                                <div className="flex items-center gap-4 mb-6">
                                    ...
                                    <h4 className="font-black text-slate-800 text-lg">Efficiency Benchmarks</h4>
                                </div>
                                <div className="h-[300px] w-full">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart data={teamChartData} barSize={32}>
                                            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                                            <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 10, fontWeight: '700', fill: '#94a3b8' }} dy={10} />
                                            <Tooltip cursor={{ fill: '#f8fafc', radius: 8 }} contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 30px -10px rgba(0,0,0,0.1)' }} />
                                            <Bar dataKey="Efficiency" radius={[8, 8, 8, 8]}>
                            ...
                            <div className="bg-white p-8 rounded-[32px] shadow-xl border border-slate-100">
                                <div className="flex items-center gap-4 mb-6">
                                    ...
                                    <h4 className="font-black text-slate-800 text-lg">Consistency Score</h4>
                                </div>
                                <div className="h-[300px] w-full">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart data={teamChartData} barSize={32}>
                                            ...
                                            <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 10, fontWeight: '700', fill: '#94a3b8' }} dy={10} />
                                            <Tooltip cursor={{ fill: '#f8fafc', radius: 8 }} contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 30px -10px rgba(0,0,0,0.1)' }} />
                                            <Bar dataKey="Consistency" radius={[8, 8, 8, 8]} fill="#8b5cf6" />
                                        </BarChart>
                                    </ResponsiveContainer>
                                </div>
                            </div>
