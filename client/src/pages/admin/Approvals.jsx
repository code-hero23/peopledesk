...
const Approvals = () => {
  ...
  const renderRequestCard = (req, type, typeLabel, color) => (
    <div
      key={`${type}-${req.id}`}
      className={`bg-white p-6 rounded-xl shadow-sm border border-l-4 transition-shadow hover:shadow-md ${
        req.status === "PENDING"
      ...
      {activeTab === "pending" && canApprove && (
        <div className="grid grid-cols-2 gap-3">
          ...
          {isGlobalBH && bhView === "others" ? (
            ...
            <>
              <button
                onClick={() => onUpdateStatus(type, req.id, "APPROVED")}
                className="w-full py-2 rounded-lg bg-green-50 text-green-700 hover:bg-green-100 font-bold text-sm transition-colors"
              >
                Approve
              </button>
              <button
                onClick={() => onUpdateStatus(type, req.id, "REJECTED")}
                className="w-full py-2 rounded-lg bg-red-50 text-red-700 hover:bg-red-100 font-bold text-sm transition-colors"
              >
                Reject
              </button>
            </>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Request Cards (Permission and leave) */}
  {/* Request Cards */}
        ...
        <div className="col-span-full">
            ...
            {bhView === 'others' && (
                <p className="mt-2 text-xs text-amber-600 font-semibold">
                    🔒 Monitoring only — you cannot approve requests assigned
                    to other Business Heads.
                </p>
            )}
        </div>
    )}

    {sortedRequests.map(({ req, type, label, color }) =>
