...
const LeadConversionWorkLogForm = ({ onSuccess }) => {
    ...
    const handleSubmit = (e) => {
        ...
        if (isOpening) {
            const payload = {
                logStatus: 'OPEN',
                process: 'Lead Conversion Opening Report',
                customFields: {
                    openingLeads: validRows
                },
                remarks: remarks
            };
            ...
            const payload = {
                logStatus: 'CLOSED',
                process: 'Lead Conversion Daily Reports Completed',
                customFields: {
                    ...existingFields,
                    closingLeads: validRows
                },
        ...
        <form onSubmit={handleSubmit} className="space-y-4 max-h-[80vh] overflow-y-auto px-1">
            <div className={`p-4 rounded-lg border mb-4 ${isOpening ? 'bg-blue-50 border-blue-100' : 'bg-orange-50 border-orange-100'}`}>
                <h4 className={`font-bold text-sm uppercase ${isOpening ? 'text-blue-800' : 'text-orange-800'}`}>
                    {isOpening ? 'Lead Conversion - Opening Report' : 'Lead Conversion - Closing Report'}
                </h4>
                <p className="text-xs opacity-75">
                    {isOpening ? 'List the conversion targets/status for today.' : 'Update the conversion results for the day.'}
                </p>
            </div>
