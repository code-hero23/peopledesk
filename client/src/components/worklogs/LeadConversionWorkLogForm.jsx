...
const LeadConversionWorkLogForm = ({ onSuccess }) => {
    ...
    const handleOpeningSubmit = () => {
        setConfirmationConfig({
            isOpen: true,
            title: 'Start Conversion Session',
            message: 'Are you sure you want to start your work session?',
            onConfirm: () => {
                const payload = {
                    logStatus: 'OPEN',
                    process: 'Lead Conversion Session Started',
    ...
    const handleClosingSubmit = (e) => {
        ...
        setConfirmationConfig({
            ...
            message: `Are you sure you want to finalize your daily lead conversion?`,
            onConfirm: () => {
                const payload = {
                    logStatus: 'CLOSED',
    ...
    if (isTodayClosed) {
        return (
            <div className="bg-emerald-50 p-8 rounded-3xl text-center border border-emerald-100">
                ...
                <p className="text-emerald-600 font-bold">Your lead conversion reports have been submitted.</p>
    ...
    if (!isTodayOpen) {
        return (
            <div className="space-y-6">
                <div className="bg-gradient-to-r from-lime-500 to-lime-600 p-6 rounded-2xl text-white shadow-lg">
                    <div className="flex items-center gap-4">
                        <div className="bg-white/20 p-3 rounded-xl">
                            <Clock size={24} className="text-white" />
                        </div>
                        <div>
                            <h3 className="font-black text-2xl tracking-tight">Conversion Opening</h3>
                            <p className="text-lime-100 text-sm font-medium">Start your work session</p>
                        </div>
                    </div>
                </div>
                <button 
                    onClick={handleOpeningSubmit}
                    className="w-full py-6 bg-lime-600 hover:bg-lime-700 text-white font-black rounded-2xl shadow-xl transition-all active:scale-95 flex items-center justify-center gap-3"
                >
                    <Plus size={24} />
                    START WORK SESSION
                </button>
