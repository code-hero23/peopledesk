...
const VendorManagementWorkLogForm = ({ onSuccess }) => {
    ...
    const handleOpeningSubmit = () => {
        setConfirmationConfig({
            isOpen: true,
            title: 'Start Vendor Session',
            message: 'Are you sure you want to start your work session?',
            onConfirm: () => {
                const payload = {
                    logStatus: 'OPEN',
                    process: 'Vendor Management Session Started',
    ...
    const handleClosingSubmit = (e) => {
        ...
        setConfirmationConfig({
            ...
            message: `Are you sure you want to finalize your daily vendor management tasks?`,
            onConfirm: () => {
                const payload = {
                    logStatus: 'CLOSED',
    ...
    if (!isTodayOpen) {
        return (
            <div className="space-y-6">
                <div className="bg-gradient-to-r from-teal-500 to-teal-600 p-6 rounded-2xl text-white shadow-lg">
                    <div className="flex items-center gap-4">
                        <div className="bg-white/20 p-3 rounded-xl">
                            <Clock size={24} className="text-white" />
                        </div>
                        <div>
                            <h3 className="font-black text-2xl tracking-tight">Vendor Opening</h3>
                            <p className="text-teal-100 text-sm font-medium">Start your work session</p>
                        </div>
                    </div>
                </div>
                <button 
                    onClick={handleOpeningSubmit}
                    className="w-full py-6 bg-teal-600 hover:bg-teal-700 text-white font-black rounded-2xl shadow-xl transition-all active:scale-95 flex items-center justify-center gap-3"
                >
                    <Plus size={24} />
                    START WORK SESSION
                </button>
